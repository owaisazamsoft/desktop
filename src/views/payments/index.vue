...
<v-row>
    <v-col cols="12">
        <v-row class="mt-3">
            <v-col cols="12" class="">
                <v-card title="Payments" subtitle="View All Payments List" class="">
                    <v-card-text>
    ...
    methods: {
        async loadItems() {
            ...
            try {
                ...
                let res = await this.userStore.getList("payments",this.filter);
                console.log(res);
                
                this.items =  res.data.map((item) => {
                    item.user = userIds[item.user] ?? '';
                   return item;    
                })
        ...
        download(){
            this.themeStore.exportToExcel("payments",this.items);
        }
